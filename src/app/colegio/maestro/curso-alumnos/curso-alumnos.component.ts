...
export class CursoAlumnosComponent implements OnInit {
  ...
  secondChildsInsert(value: any) {
    ...
    this.secondChildService.createAll(formValue)
      .then(response => {
        const archivo = $('#avatar').val();
        if (this.event) {
          const archivoss = this.event;
          const archivos = archivoss.srcElement.files;
          ...
          response.forEach(element => {
            const url = `${this.basePath}/api/recommendations/upload/${element.id}`;
