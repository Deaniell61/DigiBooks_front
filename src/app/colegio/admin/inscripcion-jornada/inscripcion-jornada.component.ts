...
export class InscripcionJornadaComponent implements OnInit {
  ...
  onItemDrop(e: any) {
    ...
    if (!this.selectedParent) {
      ...
      const existe = (this.selectedData.find(dat => {
        return dat.id === e.dragData.id;
      }));

      if (!existe) {
        const aF1 = this.selectedDate.begin.split('-');
        const aF2 = this.selectedDate.end.split('-');
    ...
    this.mainService.getMyId(values[0], id)
      .then(response => {
        ...
        this.mainService.getMyGrandChilds(this.selectedDataId)
          .then(responseChild => {
            this.selectedData = responseChild;
  ...
  makeCharge(cant: number) {
    ...
    const dates = this.selectedDate.begin.split('-');
    for (let i = 0; i < cant; i++) {
      const month = ((dates[1] * 1) + i);
      data.charges.push(
